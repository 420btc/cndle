...
import WheelSpinScreen from './WheelSpinScreen';
...
const GameScreen: React.FC = () => {
  ...
  const [lastResult, setLastResult] = useState<{ correct: boolean; message: string; coinsWon?: number; bonusAmount?: number; multiplierApplied?: number; momentumBonus?: number; momentumStreak?: number; candleSize?: number; recordMessage?: string | null } | null>(null);
  ...
  // New state to track if player has already bet on this candle
  ...
  // New state for candle size bonus message
  const [bonusMessage, setBonusMessage] = useState<string | null>(null);
  const [recordMessage, setRecordMessage] = useState<string | null>(null);
  const bonusMessageOpacity = useRef(new RNAnimated.Value(0)).current;
  ...
  const bonusMessageTimerRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const logDebugState = () => {
    ...
    console.log(`[DEBUG STATE] Wheel multiplier active: ${wheelMultiplier}`);
    ...
    console.log(`[DEBUG STATE] Market momentum: type=${momentumType}, streak=${momentumStreak}`);
  };
  ...
  useEffect(() => {
    ...
    const unsubscribeFocus = navigation.addListener('focus', () => {
      ...
      console.log('[DEBUG] Returned to game screen');
      
      // Always check for bets that need resolution when returning to the game screen
      setTimeout(() => {
        console.log('[DEBUG] Checking for unresolved bets after returning to game screen');
        checkForPendingBetsToResolve();
        checkForUnresolvedBets(); // Add this to update the hasUnresolvedBets flag
    ...
    return () => {
      ...
      if (bonusMessageTimerRef.current) clearTimeout(bonusMessageTimerRef.current);
  ...
  // Function to show bonus message notification
  const showBonusMessage = (bonusMsg: string, recordMsg: string | null = null) => {
    setBonusMessage(bonusMsg);
    setRecordMessage(recordMsg);
    
    // Animate notification in
    RNAnimated.timing(bonusMessageOpacity, {
      toValue: 1,
      duration: 300,
      useNativeDriver: true
    }).start();
    ...
    // Set timer to hide notification
    if (bonusMessageTimerRef.current) {
      clearTimeout(bonusMessageTimerRef.current);
    }
    
    bonusMessageTimerRef.current = setTimeout(() => {
      RNAnimated.timing(bonusMessageOpacity, {
        toValue: 0,
        duration: 300,
        useNativeDriver: true
  ...
  // Modified refreshCandles function to only update chart data without affecting bets
  const refreshCandles = async () => {
    try {
      ...
      // Get new candles from API - Changed from 25 to 60 to allow navigation
      const velas = await getHistoricalCandles('BTCUSDT', '1m', 60);
      
      // Store the last candle timestamp
      if (velas.length > 0) {
      ...
      // Update historical candles without affecting active bets
  ...
  const handleWebSocketMessage = (vela: Candle) => {
    ...
    if (vela.isClosed && vela.timestamp === lastCandleOpenTimeRef.current) {
      ...
      // Update market momentum based on candle direction
      if (vela.close > vela.open) {
        // Bullish candle (green)
        if (momentumType === 'bull' || momentumType === null) {
          // Continue or start bull momentum
          setMomentumStreak(momentumType === 'bull' ? momentumStreak + 1 : 1);
          setMomentumType('bull');
          console.log(`[DEBUG] Bull momentum: streak=${momentumType === 'bull' ? momentumStreak + 1 : 1}`);
        } else {
          // Direction changed from bear to bull
          setMomentumStreak(1);
          setMomentumType('bull');
          console.log(`[DEBUG] Momentum direction changed to bull: streak=1`);
        }
      } else if (vela.close < vela.open) {
        // Bearish candle (red)
        if (momentumType === 'bear' || momentumType === null) {
          // Continue or start bear momentum
          setMomentumStreak(momentumType === 'bear' ? momentumStreak + 1 : 1);
          setMomentumType('bear');
          console.log(`[DEBUG] Bear momentum: streak=${momentumType === 'bear' ? momentumStreak + 1 : 1}`);
        } else {
          // Direction changed from bull to bear
          setMomentumStreak(1);
          setMomentumType('bear');
          console.log(`[DEBUG] Momentum direction changed to bear: streak=1`);
        }
      } else {
        // Doji candle (no change)
        setMomentumStreak(0);
        setMomentumType(null);
        console.log(`[DEBUG] Doji candle: momentum reset`);
      }
  ...
  const resolverApuestaPorVela = (bet: ActiveBet) => {
    ...
    console.log(`[DEBUG] Wheel multiplier active: ${wheelMultiplier}`);
    console.log(`[DEBUG] Market momentum: type=${momentumType}, streak=${momentumStreak}`);
    ...
    if (isWin) {
      ...
      // Apply lucky bonus if active (x2) or use wheelMultiplier if > 1, or use nextBetMultiplier if > 1, otherwise use default 2x
      let betMultiplier = 2; // Default multiplier
      
      if (wheelMultiplier > 1) {
        betMultiplier = wheelMultiplier;
        setWheelMultiplier(1); // Reset wheel multiplier after use
        console.log(`[DEBUG] Applied wheel multiplier (${betMultiplier}x)`);
      } else if (luckyBonus) {
      ...
      // Calculate candle size (high - low) for the bet's candle
      ...
      const { bonusPercentage, bonusAmount: _ } = calculateSizeBonus(candleSize);
      const sizeBonus = Math.round((baseWinnings * bonusPercentage) / 100);
      
      // Calculate momentum bonus
      const momentumBonusPercentage = calculateMomentumBonus(momentumStreak);
      const momentumBonus = Math.round((baseWinnings * momentumBonusPercentage) / 100);
      
      // Total winnings with size bonus and momentum bonus
      const totalWinnings = baseWinnings + sizeBonus + momentumBonus;
      
      console.log(`[DEBUG] Base winnings: ${baseWinnings} coins`);
      console.log(`[DEBUG] Candle size: $${candleSize.toFixed(2)}`);
      console.log(`[DEBUG] Size bonus: ${bonusPercentage}% (${sizeBonus} coins)`);
      console.log(`[DEBUG] Momentum bonus: ${momentumBonusPercentage}% (${momentumBonus} coins)`);
      ...
      setLastResult({ 
        ...
        bonusAmount: sizeBonus,
        multiplierApplied: betMultiplier,
        momentumBonus: momentumBonusPercentage,
      ...
      // Lose the bet amount - for double bets, lose double theamount
    ...
    // Actualizar la apuesta en el historial global
  ...
  useEffect(() => {
    if (showResult) {
      ...
      }, 2500); // Changed to 2.5 seconds as requested
  ...
  const toggleWheelModal = () => {
    // Only allow opening the wheel if player has enough coins
    if (!showWheelModal && coins < 1000) {
      Alert.alert("Not Enough Coins", "You need 1000 coins to spin the wheel.");
      return;
    }
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.navBanner}>
          ...
          {/* Wheel Spin Button - Changed icon from sync to gift */}
          <TouchableOpacity 
            style={[
              styles.navButton, 
              coins < 1000 && styles.wheelButtonDisabled
            ]} 
            onPress={toggleWheelModal}
            disabled={coins < 1000}
          >
            <Ionicons name="gift" size={16} color={coins < 1000 ? "#666666" : "#f59e0b"} />
          </TouchableOpacity>
        ...
        <View style={styles.bannersContainer}>
          ...
          {/* Bet amount controls - Made smaller */}
          <View style={styles.betControlsRow}>
            <TouchableOpacity 
              style={[
            ...
            {/* Double Bet Toggle Button - Made smaller */}
            <TouchableOpacity 
              style={[
              ...
              disabled={hasBetOnCurrentCandle && gameState === GameState.FASE_APUESTA}
            >
              <Ionicons name="cash" size={14} color={isDoubleBet ? "#ffffff" : "#f59e0b"} />
              <Text style={[styles.doubleBetText, isDoubleBet && styles.doubleBetTextActive]}>
                {isDoubleBet ? "2X" : "2X BET"}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {wheelMultiplier > 1 && (
          <View style={styles.luckyBonusContainer}>
            ...
            <Text style={styles.luckyBonusText}>
              Wheel bonus active! Next bet has a x{wheelMultiplier} multiplier!
            </Text>
          </View>
        )}
              ...
              <View style={lastResult?.correct ? styles.winDetailsContainer : styles.lossDetailsContainer}>
                ...
                <View style={styles.candleSizeRow}>
                  ...
                  <Text style={styles.candleSizeText}>
                    Candle size: ${lastResult?.candleSize?.toFixed(2)} (Bonus: +{lastResult?.bonusAmount ? lastResult.bonusAmount : 0} coins)
                  </Text>
                </View>
      ...
      <Modal
        ...
        animationType="slide"
        transparent={true}
        onRequestClose={handleWheelModalClose}
      >
        <WheelSpinScreen onClose={handleWheelModalClose} />
      </Modal>
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  // Bet controls row - New style for the reorganized controls
  betControlsRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  // Double bet button styles - Made smaller
  doubleBetButton: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#2d2d2d',
  ...
  // Wheel button styles - Moved to navButton
  wheelButtonDisabled: {
    opacity: 0.5,
  },
